...
def suite_syracuse(nombre_depart):
    """Renvoie la suite (liste) de Syracuse à partir du [nombre_depart]"""
    s = [nombre_depart]
    n = nombre_depart
    while n != 1:
        if n % 2 == 0:
...
def plus_petit_nombre_depart_suite_syracuse_plus_longue(limit):
    """Retourne le plus petit nombre de depart inférieur à limit ayant la plus longue suite de Syracuse"""
    
    min_depart = 1
    max_long = 0

    for nombre in range(1, limit + 1):
        taille_suite = len(suite_syracuse(nombre))
